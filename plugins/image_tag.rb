  ...
  class CaptionImageTag < ImageTag
    ...
    def render(context)
      if @img
        ...
        "<figure class='#{@class}'><a href='#{@img['src']}' title='#{@img['title']}' rel='fancyGallery'' class='fancybox'><img #{@img.collect {|k,v| "#{k}=\"#{v}\"" if v}.join(" ")}></a><figcaption>#{@img['title']}</figcaption></figure>"
      else
        "Error processing input, expected syntax: {% img [class name(s)] [http[s]:/]/path/to/image [width [height]] [title text | \"title text\" [\"alt text\"]] %}"
      end
    end
